...
const CycleSpacingTime = 2000;

/** @param {NS} ns */
export async function waiter(ns, args, name, callback) {
  const target = args[0];
  const slot = Number(args[1]);
  const millis = Number(args[2]);
  const cycleTime = Number(args[3]) + 3000;
  ...
  ns.printf(
    "iStartThisManyMillisAfterGlobalCycleStarts: %s",
    iStartThisManyMillisAfterGlobalCycleStarts,
  );
  ns.write(
    logFile,
    ns.sprintf("%s - [%s:%s/%s] - Starting run\n", new Date().toISOString(), hostname, slot, name),
    "a",
  );
  ...
  if (twoSecsBeforeCycleStarts > 0) {
    ns.write(
      logFile,
      ns.sprintf(
        "%s - [%s:%s/%s] - Doing an initial wait of %s (%s)\n",
        new Date().toISOString(),
        hostname,
        slot,
        name,
        twoSecsBeforeCycleStarts.toFixed(2),
        ns.tFormat(twoSecsBeforeCycleStarts),
      ),
      "a",
    );
  ...
  while (true) {
    ...
    const message = ns.sprintf(
      "%s - [%s:%s/%s/%s] loop start: waiting %s (%s)",
      new Date().toISOString(),
      hostname,
      slot,
      cycle,
      name,
      Math.floor(millisToWait),
      ns.tFormat(millisToWait),
    );
    ns.print(message);
